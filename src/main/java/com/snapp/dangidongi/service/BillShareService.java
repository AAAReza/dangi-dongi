...
public class BillShareService {
  ...
  public void calculateBillShareInGroup(BillEntity bill, Long groupFriendId)
      throws NotFoundException {
    Page<UserEntity> usersInGroup =
        userRepository.findByUserFriendGroups_Group_Id(groupFriendId, Pageable.unpaged());
    BigDecimal shareOfAnyOne =
        bill.getAmount()
            .divide(new BigDecimal(usersInGroup.getTotalElements()), 0, RoundingMode.FLOOR);
    ...
    do {
      for (UserEntity userEntity : usersInGroup.getContent()) {
        BillShareEntity billShareEntity =
            BillShareEntity.builder()
                .bill(bill)
                .userFriendGroup(
                    userEntity.getUserFriendGroups().stream()
                        .filter(
                            userFriendGroupEntity ->
                                userFriendGroupEntity
                                    .getGroup()
                                    .getId()
                                    .equals(bill.getFriendGroup().getId()))
                        .findFirst()
                        .get())
                .build();
  ...
  public BillShareModel findById(Long id) throws NotFoundException {
    BillShareEntity billShare =
        billShareRepository.findById(id).orElseThrow(NotFoundException::new);
  ...
  public BillShareEntity getMyBillShareInGroup(Long billId, Long groupId, Long userId)
      throws NotFoundException {
    return billShareRepository
        .findByBill_IdAndUserFriendGroup_Group_IdAndUserFriendGroup_User_Id(billId, groupId, userId)
